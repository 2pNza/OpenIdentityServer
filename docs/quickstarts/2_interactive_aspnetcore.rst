...
This UI can be found in the `Quickstart UI repo <https://github.com/IdentityServer/OpenIdentityServer.Quickstart.UI/tree/main>`_.
...
You can inspect the source code `here <https://github.com/IdentityServer/OpenIdentityServer/blob/main/src/OpenIdentityServer/src/Test/TestUserProfileService.cs>`_
to see how it works.
...
.. note:: The quickstart UI auto-provisions external users. As an external user logs in for the first time, a new local user is created, and all the external claims are copied over and associated with the new user. The way you deal with such a situation is completely up to you though. Maybe you want to show some sort of registration UI first. The source code for the default quickstart can be found `here <https://github.com/IdentityServer/OpenIdentityServer.Quickstart.UI>`_. The controller where auto-provisioning is executed can be found `here <https://github.com/IdentityServer/OpenIdentityServer.Quickstart.UI/blob/main/Quickstart/Account/ExternalController.cs>`_.
